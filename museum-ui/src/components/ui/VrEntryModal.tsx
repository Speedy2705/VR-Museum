...
export default function VrEntryModal({ label = "Enter VR Museum", variant = "filled", className = "", compact = false, href = "/vr" }: Props) {
  ...
  return <Link href={href} aria-label={label} className={`inline-flex items-center justify-center gap-2 text-[11px] tracking-label whitespace-nowrap uppercase transition-colors ${compact ? "h-9 w-9 p-0 xl:h-auto xl:w-auto xl:px-5 xl:py-2.5" : "px-7 py-3.5"} ${style} ${className}`}>
