...
export default async function VrArtifactPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const mobileDetails = <div><p data-no-translate className="text-[9px] tracking-label uppercase text-stone">Artifact VR Gallery · {artifact.collection.title}</p><h1 data-no-translate className="font-display mt-3 text-3xl italic">{artifact.title}</h1><p data-no-translate className="mt-3 text-xs text-stone">{artifact.subtitle}</p><p data-no-translate className="mt-5 text-sm leading-relaxed text-charcoal/80">{artifact.description}</p></div>;
