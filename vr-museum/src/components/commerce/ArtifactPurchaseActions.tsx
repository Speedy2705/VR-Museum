...
export default function ArtifactPurchaseActions(props: Props) {
  ...
  return <><div className="mt-7 grid gap-3 sm:grid-cols-3"><button type="button" onClick={() => add(true)} className="bg-ink px-5 py-3.5 text-[10px] tracking-label uppercase text-white">Buy · ${props.price}</button><button type="button" onClick={() => add(false)} disabled={inCart} className="border border-line px-5 py-3.5 text-[10px] tracking-label uppercase disabled:bg-cream-dark disabled:text-stone">{inCart ? "In Cart" : "Add to Cart"}</button><VrEntryModal href={`/vr/${props.slug}`} label="Enter VR" variant="dark" className="w-full px-5"/></div><SignInPrompt open={prompt} onClose={() => setPrompt(false)} title="Sign in to purchase" description="Sign in to buy or add this artifact to your saved cart." returnTo={`/collections`} /></>;
}
