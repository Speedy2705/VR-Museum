...
export default function ReportFlagButton({ uploadId, artifactSlug }: { uploadId?: string; artifactSlug?: string }) {
  ...
  return (
    <>
      <button type="button" disabled={sent} onClick={() => status === "authenticated" ? setOpen(true) : setSignInPrompt(true)} className="text-[10px] tracking-label text-stone underline uppercase disabled:opacity-60">
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink/55 p-5" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
          <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="report-title" className="w-full max-w-lg bg-cream p-7 shadow-xl">
            <p className="text-[10px] tracking-label uppercase text-stone">Community safety</p>
            ...
            <label className="mt-5 block text-[10px] tracking-label uppercase text-stone">Reason
            ...
            <label className="mt-4 block text-[10px] tracking-label uppercase text-stone">Details (optional)
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" disabled={sending} onClick={() => setOpen(false)} className="border border-line px-5 py-2.5 text-[10px] tracking-label uppercase">Cancel</button>
              <button type="submit" disabled={sending} className="bg-ink px-5 py-2.5 text-[10px] tracking-label text-white uppercase disabled:opacity-60">{sending ? "Submitting…" : "Submit Report"}</button>
            </div>
          </form>
        </div>
      )}
