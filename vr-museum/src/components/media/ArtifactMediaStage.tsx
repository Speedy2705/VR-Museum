...
type Props = { title: string; image?: string; video?: string; model?: { url: string; format: "glb" | "gltf" | "obj" | "stl" | "usdz" }; lighting?: string | null; primaryMediaType?: MediaKind; fullscreen?: boolean; overlay?: ReactNode };
...
export default function ArtifactMediaStage({ title, image, video, model, lighting, primaryMediaType = "image", fullscreen = false, overlay }: Props) {
  ...
  const renderActive = () => {
    ...
    if (active === "model" && model && model.format !== "usdz") return <ModelViewer src={model.url} format={model.format} presetKey={keyFromDisplayName(lighting)} poster={image} title={title} />;
  ...
  if (fullscreen) return <ArtifactStageFullscreen viewer={panel} overlay={<>{tabs}{overlay}</>} />;
