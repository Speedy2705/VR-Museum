...
function InteractionGuide() {
  return (
    <div className="pointer-events-none absolute top-3 left-3 flex flex-wrap gap-2" aria-hidden="true">
      <div className="flex items-center gap-2 bg-black/75 px-3 py-2 text-xs text-white shadow-lg backdrop-blur-sm">
        <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
          <path d="M8 7h8a4 4 0 0 1 4 4v2a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4v-2a4 4 0 0 1 4-4Z" stroke="currentColor" strokeWidth="1.5" />
          <path d="M12 7v5M9.5 10.5 12 13l2.5-2.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      ...
      <div className="flex items-center gap-2 bg-black/75 px-3 py-2 text-xs text-white shadow-lg backdrop-blur-sm">
        <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
          ...
          <path d="m13 13 5 5M7 9h4M9 7v4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
...
export default function LightingStudioViewer({ src, format, presetKey = "raking-light", lightTemperature, lightDirection, title, poster, onReady, onError, showMuseumEnvironment = true, className = "", museumLayout = "centered", focusArtifactWithExhibit = false, plaqueOrigin, panelDetails, displayStyle = "sculpture" }: LightingStudioViewerProps) {
  ...
  }, [showMuseumEnvironment, museumLayout, focusObject, displayStyle, retryCount]);
  ...
  }, [src, format, showMuseumEnvironment, museumLayout, displayStyle, retryCount]);

  if (failed) return <div role="alert" className={`relative flex h-full items-center justify-center overflow-hidden bg-cream-dark p-8 text-center ${className}`}>{poster && <div className="absolute inset-0 opacity-35"><PlaceholderImage src={poster} alt="" label={title} sizes="(min-width: 768px) 50vw, 100vw" /></div>}<div className="relative"><p className="font-display text-2xl italic">The model is unavailable</p><p className="mt-3 text-sm text-stone">{failureMessage} You can still use the artifact photo.</p><button type="button" onClick={() => { setFailed(false); setRetryCount((value) => value + 1); }} className="mt-5 min-h-11 border border-ink px-5 py-3 text-xs tracking-label uppercase">Try loading again</button></div></div>;
  return <div className={`relative h-full min-h-64 w-full overflow-hidden bg-cream-dark ${className}`}><div ref={hostRef} role="application" tabIndex={0} className="absolute inset-0 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#b88932]" aria-label={`Interactive 3D model of ${title}. Drag to rotate, pinch or scroll to zoom.`} />{!loaded && <div className="pointer-events-none absolute inset-0 flex items-center justify-center" role="status" aria-live="polite" aria-label={`Loading 3D model: ${progress}%`}><LogoLoader label="Preparing 3D view" size="lg" progress={progress} /></div>}{loaded && <><InteractionGuide /><div className="absolute bottom-3 left-3 flex max-w-[calc(100%-1.5rem)] flex-wrap gap-2">{displayStyle === "sculpture" && <button type="button" aria-pressed={rotating} onClick={() => setRotating((value) => !value)} className="min-h-11 bg-cream/95 px-3 py-2 text-xs tracking-label uppercase shadow">{rotating ? "Stop rotation" : "Auto rotate"}</button>}{museumLayout === "details" ? <><button type="button" aria-pressed={focused === "artifact"} onClick={() => { const exhibit = environmentRef.current?.getObjectByName("artifact-exhibit"); const target = focusArtifactWithExhibit ? exhibit ?? modelRef.current : modelRef.current; if (target) focusObject(target, "artifact"); }} className="min-h-11 bg-cream/95 px-3 py-2 text-xs tracking-label uppercase shadow">{displayStyle === "framed-art" ? "Front view" : "Focus artifact"}</button><button type="button" aria-pressed={focused === "details"} onClick={() => { const display = environmentRef.current?.getObjectByName("museum-info-display"); if (display) focusObject(display, "details"); }} className="min-h-11 bg-cream/95 px-3 py-2 text-xs tracking-label uppercase shadow">View details</button><button type="button" aria-pressed={focused === "both"} onClick={resetView} className="min-h-11 bg-cream/95 px-3 py-2 text-xs tracking-label uppercase shadow">Reset view</button></> : <button type="button" aria-pressed={focused === "both"} onClick={resetView} className="min-h-11 bg-cream/95 px-3 py-2 text-xs tracking-label uppercase shadow">{displayStyle === "framed-art" ? "Front view" : "Reset view"}</button>}</div></>}</div>;
}
