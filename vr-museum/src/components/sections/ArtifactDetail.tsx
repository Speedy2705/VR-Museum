...
export default function ArtifactDetail({
  ...
  const details = (
    <div>
      ...
      <div className="mt-6 divide-y divide-line border-t border-line">{specs.map((spec) => <div key={spec.label} className="flex items-center justify-between py-3"><span className="text-xs tracking-label text-stone uppercase">{spec.label}</span><span data-no-translate className="text-sm text-ink">{spec.value}</span></div>)}</div>
      <div className="mt-5 border border-line px-4 py-3"><p className="text-xs tracking-label text-stone uppercase">{preset.name}</p><p className="mt-2 text-xs leading-relaxed text-stone">{preset.note}</p></div>
    </div>
  );
