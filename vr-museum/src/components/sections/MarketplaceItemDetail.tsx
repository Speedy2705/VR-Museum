...
export default function MarketplaceItemDetail({
  ...
  const immersiveDetails = <div><Breadcrumbs items={[{ label: "Marketplace", href: "/marketplace" }, { label: product.title }]} /><p className="mt-4 text-[9px] tracking-label uppercase text-stone">Marketplace artifact</p><h1 className="font-display mt-2 text-3xl italic">{product.title}</h1><p className="mt-2 text-xs text-stone">By {product.artist}</p><p className="mt-4 text-sm leading-relaxed text-charcoal/80">{product.description}</p><div className="mt-5 divide-y divide-line border-y border-line">{specs.map((spec) => <div key={spec.label} className="flex justify-between gap-4 py-2 text-xs"><span className="text-stone">{spec.label}</span><span>{spec.value}</span></div>)}</div></div>;
  ...
  if (product.model) return <section className="bg-ink pt-20"><ArtifactMediaStage title={product.title} image={product.image} video={product.video} model={product.model} lighting={product.lighting} primaryMediaType="model" fullscreen overlay={immersiveDetails} overlayActions={immersiveActions} immersiveDetails plaqueOrigin={product.period} panelDetails={{ uploadType: "Marketplace · Museum Presented", title: product.title, uploader: product.artist, description: product.description, material: product.material, origin: product.period, license: product.license, price: isFree ? "Free" : `$${product.price}` }} /><SignInPrompt open={signInPromptOpen} onClose={() => setSignInPromptOpen(false)} title="Sign in to add to cart" description="Your cart is saved securely to your museum account." returnTo={`/marketplace/${product.slug}`} /></section>;
  if (product.video && product.primaryMediaType === "video") return <section className="bg-ink pt-20"><ArtifactMediaStage title={product.title} image={product.image} video={product.video} lighting={product.lighting} primaryMediaType="video" fullscreen overlay={<>{immersiveDetails}<div className="mt-6">{immersiveActions}</div></>} /><SignInPrompt open={signInPromptOpen} onClose={() => setSignInPromptOpen(false)} title="Sign in to add to cart" description="Your cart is saved securely to your museum account." returnTo={`/marketplace/${product.slug}`} /></section>;
