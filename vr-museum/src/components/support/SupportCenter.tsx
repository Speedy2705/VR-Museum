...
export default function SupportCenter({ initialRequests }: { initialRequests: RequestItem[] }) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
    <form onSubmit={submit} className="border border-line p-6 md:p-8">
      ...
      <button disabled={sending} className="mt-6 bg-ink px-7 py-3.5 text-xs tracking-label uppercase text-white disabled:opacity-60">{sending ? <><LogoLoader label="Submitting request" size="sm" tone="light" showLabel={false} className="me-2 align-middle" />Submitting…</> : "Submit"}</button>
    </form>
